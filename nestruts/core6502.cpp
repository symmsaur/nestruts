...
void core6502::setpp(uint16_t new_pp)
{
	pp = new_pp;
}

uint8_t core6502::get_acc()
{
	return accumulator;
}
...
uint16_t core6502::zero(uint8_t adr)
{
	return adr;
}
...
{
	if (!val) status |= zero_flag;
	else status &= ~zero_flag;
}

void core6502::set_negative_flag(uint8_t val)
{
	if (!val) status |= negative_flag;
	else status &= ~negative_flag;
}
...
void core6502::SBC(uint8_t val)
{
	...
	set_accumulator(static_cast<uint8_t>(res & 0xFF));
	if (res > UINT8_MAX) status &= ~carry_flag;
	else status |= carry_flag;
	set_zero_flag(accumulator);
...
void core6502::ASL()
{
	set_accumulator(ASL(accumulator));
}
...
void core6502::CMP(uint8_t val)
{
	compare(accumulator, val);
}

void core6502::CPX(uint8_t val)
{
	compare(x, val);
}

void core6502::CPY(uint8_t val)
{
	compare(y, val);
}
...
{
	if (reg >= val) status |= carry_flag;
	else status &= ~carry_flag;
	if (reg == val) status |= zero_flag;
	else status &= ~zero_flag;
	// negative (a bit of a guess)
	if (reg < val) status |= negative_flag;
	else status &= ~negative_flag;
}
...
{
	if (accumulator & val) status |= zero_flag;
	else status &= ~zero_flag;
	if (0x40 & val) status |= overflow_flag;
	else status &= ~overflow_flag;
	if (0x80 & val) status |= negative_flag;
	else status &= ~negative_flag;
}

void core6502::LSR()
{
	set_accumulator(LSR(accumulator));
}
...
void core6502::ROL(uint16_t adr)
{
	bus->write(adr, ROL(bus->read(adr)));
}
...
void core6502::ROR(uint16_t adr)
{
	bus->write(adr, ROR(bus->read(adr)));
}
...
void core6502::JMP(uint16_t adr)
{
	set_pp(adr);
}
...
void core6502::STA(uint16_t adr)
{
	bus->write(adr, accumulator);
}

void core6502::STX(uint16_t adr)
{
	bus->write(adr, x);
}

void core6502::STY(uint16_t adr)
{
	bus->write(adr, y);
}
