...
std::tuple<std::shared_ptr<picture_processing_unit>, std::unique_ptr<memory_bus>, std::shared_ptr<audio_processing_unit>> load_rom(std::string filename)
{
	...
	for (auto const c : guard)
	{
		...
		if (value < 0) throw std::runtime_error("failed to read file");
		if (value != c) throw std::runtime_error("unexpected rom file header guard");
	}
	auto const prg_rom_banks = fgetc(rom);
	if (prg_rom_banks < 0) throw std::runtime_error("failed to read file");
	auto const chr_rom_banks = fgetc(rom);
	if (chr_rom_banks < 0) throw std::runtime_error("failed to read file");
	// Skip rest of header
	fseek(rom, 16, 0);
	// load PRG ROM
	...
	for (int i = 0x8000; i <= 0xFFFF; i++)
	{
		...
		int value = fgetc(rom);
		if (value < 0) throw std::runtime_error("failed reading file\n");
		if (i % 0x1000 == 0) printf("loaded: %#06x\n", i);
		uint8_t val8 = (uint8_t)value;
		bus->load_rom(static_cast<uint16_t>(i), val8);
	}
	// Load CHR ROM
	if (chr_rom_banks != 1) throw std::runtime_error("not implemented");
