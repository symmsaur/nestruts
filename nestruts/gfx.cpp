...
graphics::graphics()
{
	...
	window = SDL_CreateWindow("Nestruts", SDL_WINDOWPOS_UNDEFINED, 
...
void graphics::draw_pixel(int nes_x, int nes_y, uint8_t color)
{
	...
	auto blue = color * 64;
	uint32_t color_argb = ((red + (color ? 127 : 0)) << 16) + (green << 8) + blue;
	...
	for (int x = nes_x * fatness; x < nes_x * fatness + fatness; x++)
	{
		...
		{
			*(static_cast<uint32_t*>(surf->pixels) + x + y * surf->pitch / sizeof(uint32_t)) = color_argb;
		}
	}
